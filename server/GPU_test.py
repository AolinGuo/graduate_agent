...
def main():
    ...
    for i in range(gpu_count):
        ...
        print(f"物理GPU {i}：{props.name} | 总显存：{props.total_memory/1024/1024/1024:.2f}GB | PCI总线ID：{props.pci_bus_id}")
    print("==========================\n")
    ...
    try:
        ...
        while True:
            ...
            print(f"[{timestamp}] 物理GPU {gpu_id} | 已分配显存：{used_mem:.2f}GB | 预留显存：{reserved_mem:.2f}GB | 唯一PCI ID：{current_props.pci_bus_id}")
            time.sleep(5)
