...
VLLM_CONFIG = {
    "model": {
        "tensor_parallel_size": 2,  # 多PU
        "dtype": "bfloat16",  # 或 "float16"
        "max_model_len": 4096,  # 最大序列长度
        "gpu_memory_utilization": 0.9,  # GPU显存利用率
        "trust_remote_code": True,
        "enforce_eager": True,  # 禁用CUDA图，使用eager模式，解决"illegal instruction"错误
    },
    "sampling": {
        "temperature": 0.7,
        "top_p": 0.9,
    ...
    def load_model(self):
        ...
        try:
            ...
            self.llm = LLM(
                ...
                enforce_eager=model_config.get("enforce_eager", True),  # 禁用CUDA图捕获，避免"illegal instruction"错误
            )
