...
def worker_process(gpu_id, subset_docs, worker_id):
    ...
    print(f"👷 [Worker {worker_id}] 启动: 使用物理 GPU {gpu_id}, 需处理 {len(subset_docs)} 条文档")
    try:
...
def recursive_merge(paths, embeddings, level=0):
    ...
    for i in range(0, len(paths), MERGE_GROUP_SIZE):
        ...
        try:
            base_db = FAISS.load_local(group[0], embeddings, allow_dangerous_deserialization=True)
            
            # 2. 依次合并组内剩余的
            for sub_path in group[1:]:
                next_db = FAISS.load_local(sub_path, embeddings, allow_dangerous_deserialization=True)
...
def main():
    ...
    for item in raw_data:
        if "page_content" in item:
            all_docs.append(Document(page_content=item["page_content"], metadata=item.get("metadata", {})))
